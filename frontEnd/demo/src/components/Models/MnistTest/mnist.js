 ...
 *     batches & epoch end.
 */
async function train(model, onIteration, batchSize) {
  ...
  await model.fit(trainData.xs, trainData.labels, {
    ...
    callbacks: {
      onBatchEnd: async (batch, logs) => {
        ...
        lineChart.addDataPoint([logs.loss,logs.loss]);
        d3.select("#loss-train").text(logs.loss);
        d3.select("#loss-test").text(logs.loss);
        d3.select("#iter-number").text(batch);
