    ...
    readFile = (e)=>{
        ...
        reader.onload = function(e) {
          ...
          for(let i = 0;i < n.length; i++){   
            for(let j = 0; j < n[i][0].value.length;j++){
                if(n[i][0].value[j] != null && typeof(n[i][0].value[j]) == "number"){
                  max=min=n[i][0].value[j];
                  temp = 1;
                  break;
                }
            }
            if(temp != 1){
              ...
              for(let j = 0; j < n[i][0].value.length;j++){
                ...
                for(w =0; w < stat[c]['value'].length; w++){
                  if(stat[c]['value'][w].name == n[i][0].value[j]){
                    tep = 1;
                    break;
                  }
                }
                if(tep == 1){
                  ...
                  stat[c]['value'][w]['name'] = n[i][0].value[j];
            ...
            total=0;
            for(let j = 0; j < n[i][0].value.length;j++){
              if(n[i][0].value[j] > max) max = n[i][0].value[j];
              if(n[i][0].value[j] != null && n[i][0].value[j] < min) min = n[i][0].value[j];
              total+=n[i][0].value[j];
            }
            let med = new Array;
            for(let j = 0; j < n[i][0].value.length;j++){
            ...
            for(let j = 0; j < n[i][0].value.length;j++){
              for(let k = j+1; k < n[i][0].value.length;k++){
                if(med[j] > med[k]){
                  tep = med[j];
                  med[j] = med[k];
                  med[k] = tep;
            ...
            tep = Math.floor(n[i][0].value.length/2);
            stat[c] = new Array();
            stat[c]['label'] = fieldNameArray[0][i];
            stat[c]['max']  = max;
            stat[c]['min'] = min;
            stat[c]['average'] = total/n[i][0].value.length;
