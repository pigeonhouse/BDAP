...
class HdfsFileTreeModal extends React.Component {
    ...
    onLoadData = treeNode => new Promise((resolve) => {
        ...
        fetch(`https://result.eolinker.com/MSwz6fu34b763a21e1f7efa84a86a16f767a756952d0f95?uri=localhost:8888/hdfs/${treeNode != null ? treeNode.props.title : ''}`, init).then((res) => {
            if (res.status === 200) {
                res.json().then((res) => {
    ...
    render() {
        // 取出父组件传递过来的参数
        const { title, visible, handleOk, handleCancel } = this.props;
        return (
            <Modal
                title={title}
                visible={visible}
